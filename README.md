...
| 2 | `C:\에이전트\01_KIT\starter-kit` 에 표준 설치한다. `C:\에이전트\내자료\` 도 같이 만든다 | 수 초 |
...
- `C:\에이전트\01_KIT\starter-kit\README.md` — 폴더 설명과 카드 사용법
- `C:\에이전트\내자료\` — **제안서 3개·블로그 글 3개·회사 로고**를 수업 전에 넣어 두는 곳 (`제안서\` `블로그\` `로고\` `프로필\`)
